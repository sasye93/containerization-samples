#!/bin/sh
echo "Status for stack thesis.samples.chat.MultitierApi:"
echo "--------------------------------------------------------------------------"
echo "Status of service server:"
echo "----------------- start of service error output --------------------------"
ID="$(docker service ps --filter "desired-state=running" --format "{{.ID}}" thesis_samples_chat_multitierapi_server)"
if [ -z "${ID}" ]; then
...
echo "----------------- start of service error output --------------------------"
ID="$(docker service ps --filter "desired-state=running" --format "{{.ID}}" thesis_samples_chat_multitierapi_client)"
if [ -z "${ID}" ]; then
  echo "Service not running."
  else
    OUT="$(docker service logs --raw --details --timestamps "$ID") $(docker ps --no-trunc --format "{{.Error}}" --filter "id=$ID")"
