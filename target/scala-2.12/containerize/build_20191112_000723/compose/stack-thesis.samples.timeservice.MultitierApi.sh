...
ERR=0
docker images -q thesis_samples_timeservice_service > /dev/null 2>&1
if [ $? -ne 0 ]; then
  ERR=1
  echo "Could not find image thesis_samples_timeservice_service, which is needed by service service. This service will not start up."
fi
docker images -q thesis_samples_timeservice_client > /dev/null 2>&1
if [ $? -ne 0 ]; then
  ERR=1
  echo "Could not find image thesis_samples_timeservice_client, which is needed by service client. This service will not start up."
fi
if [ $ERR -ne 0 ]; then
