...
@service object PipelineMain extends App {
  loci.multitier start new Instance[Pipeline.Folder](
    connect[Pipeline.Mapper] { TCP(1095).firstConnection })
}
@service object PipelineMain2 extends App {
  loci.multitier start new Instance[Pipeline.Mapper](
    connect[Pipeline.Folder] {
      TCP("localhost", 1095)
    } and
      connect[Pipeline.Filter] {
        TCP(1096).firstConnection
      })
}
@service object PipelineMain3 extends App {
  loci.multitier start new Instance[Pipeline.Filter](
    connect[Pipeline.Mapper] {
      TCP("localhost", 1096)
    } and
      connect[Pipeline.Input] {
        TCP(1097).firstConnection
      })
}
@service object PipelineMain4 extends App {
